...
Future<void> main() async {
  ...
  List<String> lines = <String>['LG06', 'T08', 'T10', 'T13', 'T22', 'T24', 'T25', 'T27', 'T29', 'T39'];
  MultiGraph<StopVertex, TransitEdge> graph = await graphService.getFullTransitsGraphByLines(startTime, lines);

  print('small stops count: ${graph.keys.length} ${graph.edges.length}');
  ...
  //
  //   await FileUtils.writeLocalFile('small_result_dijkstra.csv', dijkstraResults.join('\n'));
  //   await FileUtils.writeLocalFile('small_result_a_star.csv', aStarResults.join('\n'));
  //   await FileUtils.writeLocalFile('small_result_bfs.csv', bfsResults.join('\n'));
  //   await FileUtils.writeLocalFile('small_result_dfs.csv', dfsResults.join('\n'));
  // }
}
