...
class AStar {
  ...
  PathSearchResult search(TransitSearchRequest transitSearchRequest) {
    ...
    while (_unvisitedStopsQueue.isNotEmpty) {
      ...
      for (TransitEdge neighborEdge in neighbors) {
        ...
        double heuristicCost = _calcHeuristicCost(neighborVertex) * 0.005;
        double edgeCost = neighborEdge.calcCost(transitSearchPosition) + heuristicCost;
  ...
  double _calcHeuristicCost(StopVertex stopVertex) {
    return Haversine.calcDistanceInMeters(stopVertex, _targetVertex);
  }
